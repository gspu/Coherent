...
.PP
The following document describes how to install BBL Typographic's
sample package of images onto your \*(UN or \*(CO system.
...
.BR troff .
.SH "Installing the Sample Package"
.PP
The BBL sample package consists of files compressed by the GNU
utility
.BR gzip ,
and are stored on a
.B tar
...
command line.
.PP
For example, assume that you have installed the sample package
into directory
.BR /usr/bbl .
The following example, called
.BR test.eps ,
gives a brief program that prints a capital `S' from this package:
.sp \n(pDu
.nf
.ft L
.vs 9p
	...
	This mass of text precedes the image.
	This mass of text precedes the image:
	.PH 1 1 /usr/bbl/bbl_sample/was.bnd
	.PP
...
.DE
Note that the \fB.eps\fR macros do not set up the page; they only position
images.
...
Example call:
.DM
	.PF 1 1 /usr/bbl/bbl_sample/was.bnd
.DE
.IP \fB.PL\fR
...
Example call:
.DM
	.PL 1 1 /usr/bbl/bbl_sample/was.bnd
.DE
.IP \fB.PB\fR
...
Example call:
.DM
	.PB 1 1 /usr/bbl/bbl_sample/was.bnd
.DE
.IP \fB.PR\fR
...
Example call:
.DM
	.PR 1 1 /usr/bbl/bbl_sample/was.bnd
.DE
.PP
