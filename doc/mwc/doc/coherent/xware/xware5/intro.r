...
.B tcl
scripting language.
The widgets consist of bitmaps and code that together implements commonly
used X objects, such as windows, buttons, sliders, menus, and
font-management tools.
The widgets are designed to resemble thoses used by the Motif window
manager.
It also implements some extensions to the
.B tcl
scripting language; in particular, it implements a method by which
X processes can communicate with each other.
.IP
.B tk
widgets are implemented in the form C routines that you can link into
C programs.
...
Although
.B wish
is an interpreter, like the shells
.B sh
and
.BR ksh ,
it is quite fast.
...
.PP
The first part of installation involves moving the files onto your
system:
.IP \(bu 0.3i
...
.PP
Please note that by default, the shells
.BR tclsh ,
.BR wish ,
and
.B xf
...
.PP
Finally, the source files for the
.B tk
manual pages are in archive
.BR tk\-doc.gtz ,
which is shipped on disk 1 of this package.
All of the formatted TK manual pages are installed when you install this
package onto your system.
Should you wish to have the sources for the TK manual pages on your system,
mount disk 1 of the release, copy archive
.B tk\-doc.gtz
into the directory which holds TK, and then run the command:
.DM
	gtar -xvzf tk\-doc.gtz
.DE
.SH "Copyrights"
.PP
