...
1.1
date	94.05.04.22.03.41;	author rcsadmin;	state Exp;
branches;
next	;
...
@/*
 * build.c
 * last hacked Fri Jan 28 14:05:24 1994 CST by Louis
 *
 ...
 *
 * The above comments are mostly wrong.  I just changed the usage which
 * was missing the -u flag.  To be complete, you also need some
 * of the "new device stuff" going. Caveat utilitor. -Louis  1/94
 */
...
/* Manifest constants. */
#define	VERSION		"5.0"
#define	USAGE		"Usage: /etc/build [ -duv ]\n"
...
/*
 * **NB** 
 * We now need more than 15 megs on a root partition. -Louis 1/94
 */
...
/*
 * **NB**
 * Steve claims that drive manufacturers use 10^6 instead of
 * 2^20 for a megabyte.  I know Seagate does, but for some
 * reason I thought that they were the only ones to propagate
 * this type of brain damage.  However, since Steve so kindly 
 * reminded me that he is older and wiser, I will leave it as
 * 10^6.  -Louis 1/94
 */
#if 0
#define	meg(sec)	((double)sec * BSIZE / 1000000.)
#else /* Young smart-ass punk method. */
#define meg(sec)	((double)sec * BSIZE / 1048576.)
#endif
...
"Be patient.  This takes a few minutes.\n"
		...
		for (i = 0; i < ndevices; i++) {
			...
			if (notflag(i, F_COH))
				continue;	/* scan only COH partitions */
...
"You can skip scanning it for bad blocks if you choose.\n",
			...
			else { /* COH part w/out filesystem */
			  if(isflag(i, F_ATDEV))
			    printf(
...
"may skip scanning it for bad blocks if you choose.\n",
			...
			setflag(i, F_SCAN);
			/* This used to use rawname(i, 1), but this
			 * is no longer the right way.
			 */
			sprintf(cmd, "/etc/badscan -v -o %s %s %s",
				protoname(i), rawname(i, 0), xname(i, 1));
...
"previously existing data on the partition.\n\n"
	...
	for (i = 0; i < ndevices; i++) {
		...
		/*
		 * If it's not a COH partition, skip it.
		 */
		if (notflag(i, F_COH))
			continue;

		/*
		 * If it is an AT device, non-scanned partition with
		 * no filesystem, warn them that they should scan it first.
		 */
		if(isflag(i, F_ATDEV) && notflag(i, F_SCAN) && notflag(i, F_FS)) {
			printf("WARNING: blank AT partition %d not scanned.\n", i);
			printf("Non-IDE AT drive partitions should be scanned.\n");
		}
 ...
 * If flag and not one of the built-in AT device names,
 * the device is in /tmp/dev rather than /dev.
 * Changed to do badscan on SCSI devices by Louis.
 */
char *
