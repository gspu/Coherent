...
1.2
date	94.10.21.18.30.11;	author rcsadmin;	state Exp;
branches;
next	1.1;

1.1
date	93.11.04.18.51.36;	author rcsadmin;	state Exp;
branches;
next	;
 ...
 * fdisk.c
 *
 * Last hacked: 07/06/92, COHERENT 386
 *
...
unsigned char	drive_x[] = "Drive x";  /* Needs to be written on	*/
...
main(argc, argv) int argc; char *argv[];
{
  ...
  if (--argc == 0) {
    ...
    fatal("No partition devices were specified.");
  }
...
/*
 * If 286, copy /coherent to /tmp/coherent and
...
atpatch()
{
  ...
  if (!Bflag)			/* Special patching required? */
    return;
...
change_part(n) int n;
{
 ...
 getbase:
  ...
  if (base != obase || size != osize) {
    ...
    if (p->p_ecyl < p->p_bcyl)
      p->p_ecyl = 1023;

    p->p_ehd = h;
    p->p_esec = ((((c % 1024) < p->p_bcyl ? 1023 : (c % 1024)) >> 2)
      & CYLMASK) | s;
  }
  printf("\n");
}
...
check_chs(p, flag) FDISK_S *p; int flag;
{
 ...
 again:
  ...
  if (c >= ncyls
    ...
    ++nmods;
    if (flag) {
      p->p_bcyl = (nc % 1024) & 0xFF;
      p->p_bhd = nh;
      p->p_bsec = (((nc % 1024) >> 2) & CYLMASK ) | ns;
    } else {
      p->p_ecyl = (nc % 1024) & 0xFF;
      p->p_ehd = nh;
      p->p_esec = (((nc % 1024) >> 2) & CYLMASK ) | ns;
    }
  }
}
...
bad_deal:
    printf("\nWARNING: The BIOS says your disk has %d cylinders.\n"
	   "\nYou have 3 choices:\n"
	   "  1) You may force COHERENT to only use the first 1024 cylinders\n"
	   "     resulting in wasted disk space but full compatibility\n"
	   "     with DOS fdisk and other operating systems.\n"
	   "\n  2) You may continue and use all the cylinders.  This may\n"
	   "     cause problems with DOS fdisk and other operating systems.\n"
	   "\n  3) You may quit now and change the CMOS settings for your\n"
	   "     hard drive by running your BIOS hard disk setup. New BIOSes\n"
	   "     support a translation mode whereby the drive parameters are\n"
	   "     remapped so they fall under 1024 cylinders.  Choosing this\n"
	   "     option may cause you to lose all data if the disk is\n"
	   "     already partitioned.\n\n",
	   ncyls);
...
"if you do not know the correct values.\n"
	...
	if (!yes_no("Are you sure you want to change the disk"
		    "parameter values"))
		return;
...
char *prompt; register long defval, min, max;
{
  long val;
  char *s;
...
text
@a14 2
 * If no device argument is given, fdisk supplies "/dev/at[01]x"
 * as appropriate.
d17 1
d64 115
a178 119
