...
char	*usemsg		= "\
Usage: dump [bdfsuv0123456789 [args] [filesystem]]\n";
...
char *argv[];
{
	...
	if (vflag) {
		if (ddate == 0)
			printf("dump: since epoch\n");
		else
			printf("dump: since %s", ctime(&ddate));
	}
	dbclaim(5);
	sync();
...
ddflush()
{
	...
	if (used >= length) {
		fclose(dtp);
		printf("dump: mount next tape, hit newline ...");
		if (gets(buf) == NULL)
			exit(1);
		if ((dtp = fopen(dtn, "w")) == NULL)
			fatal("%s: cannot open dump file", dtn);
		dumpheader();
		used = ddlen(sizeof(struct dumpheader)) + ddlen(ddnbuf);
	}
