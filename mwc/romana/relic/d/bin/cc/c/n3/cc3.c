 ...
 * C compiler intermediate language files.
 * It is a machine dependent program, however
 * most, if not all, of the machine dependencies
 * are in tables. It understands all favours of
 * intermediate files (that is, it understands
 * both trees and CODE nodes!).
 */
#ifdef	vax
...
work3()
{
	...
	for (;;) {
		...
		case SBODY:
		{
			register int deflab, ncases, clabel;
			register ival_t cvalue;
