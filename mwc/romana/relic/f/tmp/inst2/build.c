...
/* get rid of this... this makes fdisk ALWAYS try to open 2 AT drives even
 * if only one or none are present.
 * Bob H. (8/10/93)
 */
/*
 * Device table.
...
"coh.B4.420. After this the remainder of diskette #1 will be copied.\n\n"
	...
	sprintf(cmd, "/bin/cp %s %s /mnt/conf", IDCMDFILE, IDVARFILE);
	sys(cmd, S_FATAL);
	...
	/* If a mkdev file was created for tape devices for the hai driver,
	 * copy it to /etc/conf/hai/mkdev so that when the final kernel is
	 * finally built, the mkdev script will automatically be invoked to
	 * create the appropriate tape device nodes.
	 * Bob H. (August 13, 1993)
	 */

	if(exists("/tmp/haimkdev")){
		sys("cp /tmp/haimkdev /mnt/etc/conf/hai/mkdev",S_NONFATAL);
	}
...
again:
	...
	if (meg(devices[root].d_size) < (double)NEEDSIZE) {
		if (update) {
			printf(
"\n"
"Your current COHERENT 4.0.x root filesystem is too small to contain the\n"
"COHERENT 4.2.0 update along with the on-line manual pages and the on-line\n"
"dictionary.  If you wish to have the COHERENT 4.2.0 on-line manual pages and\n"
...
/* Call add_devices to read /tmp/devics for ALL types of drives. Previously,
 * the 2 possible AT drives were hard coded so that they would always be read,
 * even if only one or none were present.
 * Bob H. (8/10/93)
 */
...
rootpatch()
{
	 ...
	 */
	/* For third kernel. */
	sprintf(line, "unsigned long\t%s = 0x%lx;", BAR_BAR, atol(serialno));
	cohtune_ent("cohmain", BAR_BAR, line);

	sprintf(line, "unsigned long\t%s = 0x%lx;", BAR_ENTRY, atol(serialno));
	cohtune_ent("cohmain", BAR_ENTRY, line);
