...
#ifndef lint
static char rcsid[] = "$Id: gzip.c,v 0.14 1993/02/24 18:23:13 jloup Exp $";
#endif
...
int text_mode;        /* set if original file is a text file (in variable
			 record format for VMS). Must stay 0 if unknown. */
...
local void treat_stdin()
{
    ...
    text_mode  = 0; /* type unknown */
#else
    if (fstat(fileno(stdin), &istat) != 0) {
	error("fstat(stdin)");
    } 
    time_stamp = istat.st_mtime;
    text_mode = TEXT_MODE(istat);
#endif
 ...
 *   text_mode has been initialized if decompressing.
    ...
    char *name;
{
   ...
   if ((v = strrchr(suffix, SUFFIX_SEP)) != NULL) *v = '\0';
#endif
    strlwr(suffix);
