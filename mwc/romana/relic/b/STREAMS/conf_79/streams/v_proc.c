...
#define	PNODE_TO_PROC(pnode)	__DOWNCAST (PROC, p_nigel, \
				      __DOWNCAST (dpdata_t, dp_pnode, pnode))
...
#endif
{
	...
	/*
	 * Since Coherent's signal-checking causes a longjump out to the
	 * top-level, we'll check for signals here ourself.
	 */

	if (flag == SLEEP_INTERRUPTIBLE && nondsig ()) {
		/*
		 * We have pending signals that need processing, dequeue us
		 * from the plist, ulock the plist, and return an appropriate
	...
	for (;;) {
		...
		x_sleep (pnode, priority, slpriNoSig, "STREAMS");
...
#if	__USE_PROTO__
__LOCAL__ void (KERNEL_SIGNAL) (dpdata_t * dpdatap, int sig)
#else
__LOCAL__ void
...
{
	/*
	 * Since the Coherent process management system is ... not very
	 * sophisticated, and fixing it is less important than getting STREAMS
	 * out the door, we punt on some issues such as process reference
	 * counts and state checking. This is just glue to the old, broken
         * system.
	 */
...
#if	__USE_PROTO__
int (MAKE_SLEEPING) (plist_t * plistp, int priority, int flag)
#else
int
