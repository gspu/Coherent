...
#define FT_ACK_TRIES	5	/* max # of tries for ACK to QIC rpt cmd*/
...
#if MWC_FT
{
	...
	fdcDrvSelect(unit, FDC_MOTOR_ON);		/* 1=motor on			*/
...
printf("zzz ");
	/* Now wait a second. */
	timeout(&ft.ft_tim, HZ, wakeup, &ft.ft_wakeMeUp);
	x_sleep(&ft.ft_wakeMeUp, pritape, slpriSigCatch, "ftRstWt");
printf("!! ");
...
printf("get drive status failed ");
	...
	xlDbPrintStat(ft.ft_report);

	if (ftCmd(QIC_CMD_ECD)) {
printf("get error code failed ");
		u.u_error = EIO;
		f.open = 0;
		setFtIntr(0);
		return;
	}
	xlDbPrintErr(ft.ft_report);
 ...
 * Reset is done keeping the unit in question selected.
 ***********************************************************************/
static void
ftResetFDC(unit)
{
...
int unit;
{
	...
	/* Toggle FDC reset line, then wait for its interrupt.		*/
	ft.ft_wakeMeUp = 1;
	fdcResetSel(unit, 1);		/* Reset, preserving selection	*/
	x_sleep(&ft.ft_wakeMeUp, pritape, slpriSigCatch, "ftSelect");
