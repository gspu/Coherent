 ...
 * This is needed for machines which lack immediate addressing modes.
 */
...
/*
 * Data structure for remembering the constants in the pool.
 * A chain of these is built up as the compilation proceeds.
 * The pool entries are written out as new entries are added.
 */
typedef	struct	pool	{
	struct	pool	*p_pp;			/* Link			*/
	int		p_op;			/* Node op		*/
	int		p_type;			/* Node type		*/
	int		p_lab;			/* Literal label	*/
	union	{
		ival_t	p_ival;			/* Value of ICON	*/
		lval_t	p_lval;			/* Value of LCON	*/
		dval_t	p_dval;			/* Value of DCON	*/
		struct	{
	...
	if (op==ICON||op==LCON||op==DCON||op==LID||op==GID||op==ADDR) {
		...
		if (pp==NULL && (pp=(POOL *)malloc(sizeof(POOL))) != NULL) {
			...
			if (op == ICON)
				...
				memcpy(pp->pd.p_dval, tp->t_dval, sizeof(dval_t));
			else if (op == LID) {
...
#if OVERLAID
/*
 * Free all of the entries in the literal pool.
 * Called at the end of the code generator phase.
 */
freepool()
{
	register POOL	*p1;
